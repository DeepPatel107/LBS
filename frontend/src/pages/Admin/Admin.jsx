...
const Admin = () => {
    ...
    try {
      ...
      switch (selectedAction) {
        ...
        case 'Find Book by Title':
          url = `/admin/title/${formData['Book Title']}`;
          method = 'GET';
          break;
      ...
      }else if (selectedAction === 'Delete Book by Title') {
  ...
  try {
    ...
    return (
      <div className="issued-books-grid">
        {books.map((book, index) => {
          const issuedCopy = book.copiesInfo?.[0];
          return (
    ...
    return <pre className="admin-response-box">User does not exist or no books issued by user</pre>;
  }
};
...
const renderUsersWhoIssuedBookGrid = () => {
  try {
    ...
    return <pre className="admin-response-box">No user Issued that Book or book does not exist</pre>;
  }
};
...
const renderOverdueBooksGrid = () => {
  try {
    ...
    return (
      <div className="issued-books-grid">
        {books.map((book, index) => {
          const overdueCopy = book.copiesInfo?.[0]; // assuming 1 copy per book is overdue
          return (
            <div key={index} className="book-card3">
              <h4>{book.title}</h4>
              <p><strong>Student ID:</strong> {overdueCopy?.studentId}</p>
              <p><strong>Issued On:</strong> {new Date(overdueCopy?.issueDate).toLocaleDateString()}</p>
              <p><strong>Was Due On:</strong> {new Date(overdueCopy?.returnDate).toLocaleDateString()}</p>
    ...
    return <pre className="admin-response-box">No overdue Books</pre>;
  }
};

const renderBookByTitle = () => {
  try {
    const book = JSON.parse(responseMsg);
    if (!book) return null;

    return (
      <div className="issued-books-grid">
        <div className="book-card3" style={{ gridColumn: "span 2" }}>
    ...
    return <pre className="admin-response-box">No book found with this title</pre>;
  }
};
...
const renderSimilarBooksGrid = () => {
  try {
    ...
    return <pre className="admin-response-box">Sorry !!</pre>;
  }
};
...
const renderAllBooksGrid = () => {
  try {
    ...
    return <pre className="admin-response-box">Invalid response format</pre>;
  }
};
